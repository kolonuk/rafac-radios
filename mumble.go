...
// NewMumbleBackend initializes a new Mumble integration handler based on environment variables.
...
// Start initiates the connection to the external Mumble server if the backend is enabled.
...
// Broadcast sends audio to the Mumble server.
// lessonID and frequency are intended to map to specific Mumble channels for isolation.
func (m *MumbleBackend) Broadcast(lessonID, frequency string, audio []byte) {
...
// RightChoice returns an evaluation of whether Mumble is appropriate for the current deployment.
